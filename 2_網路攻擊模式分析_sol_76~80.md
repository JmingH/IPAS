...
```
```
77. Elizabeth is investigating(調查) a network breach(違反) at her company. She discovers a program that
...
What best describes(描述) this attack?
A. Logic bomb 邏輯炸彈
...
```
```
78. Zackary is a malware(惡意軟體) investigator(研究員) with a cybersecurity(網路安全) firm. He is investigating(調查) malware
that is able to compromise(妥協) a target program by finding null references(引用) in the target program
and dereferencing(取消引用) them, causing an exception to be generated(產生). 
What best describes
this type of attack?
...
routers that are not documented(紀錄). He is concerned that if they are not documented(紀錄), they
