...
MX記錄（MX 紀錄）：RFC 1035定義，電子郵件互動記錄引導域名到該域名的 郵件傳輸代理（MTA, Message Transfer Agents）列表。
...
```
```
網際網路安全協定（英語：Internet Protocol Security，縮寫：IPsec）是一個協定套件，透過對IP協定的封包進行加密和認證來保護IP協定的
網路傳輸協定族（一些相互關聯的協定的集合）。
IPsec主要由以下協定組成：
一、認證頭（AH），為IP資料報提供無連接資料[完整性]、訊息認證以及防重放攻擊保護；
二、封裝安全載荷（ESP），提供[機密性]、資料來源認證、無連接[完整性]、防重放和有限的傳輸流（traffic-flow）機密性；
...
1.強制存取控制（Mandatory Access Control, MAC)：以管理者授權為基礎，所有的資訊元件都需要經過管理者授權，才能被使用者所存取。

2.自由存取控制（Discretionary Access Control, DAC)：以創建資訊元件的元件持有者授權為基礎，不需要經過管理者授權，
  持有者授權可以決定使用者對於資訊元件的存取權限。

3.角色存取控制（Role-base Access nControl）：存取權限與使用者角色相依，資訊元件的存取被授權給角色，使用者需要先取得角色身份，
...
許多較早的或配置錯誤的XML處理器評估了XML文件中的外部實體引用。攻擊者可以利用外部實體竊
...
中執行腳本，並劫持用戶會話、破壞網站或將用戶重定向到惡意站點。
...
（parameter）來給值，這個方法目前已被視為最有效可預防SQL注入攻擊的攻擊手法的防禦方式。
```

```
[68]有一種資安風險的描述為： 「因為開發者暴露了內部檔案、檔案夾、金鑰、或資料庫的紀錄，來作為 URL 或是 Form 的參數，
使攻擊者可藉 由操作這些參數擅自進入其他 Objects 中」。此為下列何項風險的描述？ 
(A) 跨站腳本攻擊（Cross-Site Scripting） (A) API 未受防護（Underprotected APIs） 
(B) 注入攻擊（Injection） (D) 無效的存取控制（Broken Access Control）
```
```
A5:2017-失效的訪問控制 Broken Access Control
未對通過身份驗證的用戶實施恰當的訪問控制。攻擊者可以利用這些缺陷訪問未經授權的功能或數
...
SQL注入（英語：SQL injection），也稱SQL隱碼或SQL注碼，是發生於應用程式與資料庫層的安全漏洞。簡而言之，是在輸入的字串之中夾帶SQL指令，
在設計不良的程式當中忽略了字元檢查，那麼這些夾帶進去的惡意指令就會被資料庫伺服器誤認為是正常的SQL指令而執行，因此遭到破壞或是入侵。
```
```
...
在需要填入數值或資料的地方，使用參數（parameter）來給值，這個方法目前已被視為最有效可預防SQL注入攻擊的攻擊手法的防禦方式。
```
...
它允許惡意使用者將程式碼注入到網頁上，其他使用者在觀看網頁時就會受到影響。這類攻擊通常包含了HTML以及使用者端手稿語言。
```
```
...
```
```
[23]我們都知道要防止 XSS 跨網站指令碼攻擊必須過濾特殊字元，請問列何者不是我們應該過濾的特殊字元？ 
(A) # (B) & (C) “ (D) || 
```
...
是一種挾制用戶在目前已登入的Web應用程式上執行非本意的操作的攻擊方法。跟跨網站指令碼（XSS）相比，XSS 利用的是用戶對指定網站的信任，
...
(A) 中午 12 點執行完全備份，晚上 20 點進行差異備份 (D) 中午 12 點執行差異備份，晚上 20 點進行完全備份 
(C) 上午 10 點執行完全備份，下午 15 點進行差異備份 (E) 上午 10 點執行差異備份，下午 15 點進行完全備份 
```
### 備份管理作業
...
(B) 規劃備份作業應包含系統設定、應用程式及資料庫等項目 (D) 備份資料需排定執行資料回復測試，並將測試結果記錄於本機紀 錄檔 
```
...
應用層防火牆
  ...
  的所有封包，並且封鎖其他的封包(通常是直接將封包丟棄)。理論上，這一類的防火牆可以完全阻絕外部的資料流進受保護的機器裡。
代理服務
