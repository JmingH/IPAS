...
1.[強制]存取控制（Mandatory Access Control, MAC)：以管理者授權為基礎，所有的資訊元件都需要經過管理者授權，才能被使用者所存取。

2.[自由]存取控制（Discretionary Access Control, DAC)：以創建資訊元件的元件持有者授權為基礎，不需要經過管理者授權，
...
駭客就有機會在你不知不覺的情況下，透過漏洞傳送釣魚網站，假訊息，下載惡意檔案、應用程式或APP到你的終端裝置（手機或平板），傷害裝置或竊取資訊。

https://zh.wikipedia.org/zh-tw/%E9%BB%91%E6%B4%9E%E6%94%BB%E6%93%8A
封包捨棄攻擊或黑洞攻擊是在電腦網路中，一種藉由丟棄封包的阻斷服務攻擊。這種攻擊可以是選擇性（如：對於一網路位址，每n個封包和每t秒，或亂數選擇一定比例的封包，也稱作灰洞攻擊）或成批的（如：捨棄所有封包）。
```
